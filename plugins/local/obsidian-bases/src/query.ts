...
import type { FilterExpression, FilterFn, BaseConfig, SortConfig, PropertyValue } from "./types.ts"
...
function normalizeFileRef(value: unknown): string {
  ...
  return ref.replace(/\\/g, "/").replace(/\.md$/i, "").toLowerCase()
}

function fileRefCandidates(file?: QuartzPluginData): Set<string> {
...
function matchesFileRef(value: unknown, file?: QuartzPluginData): boolean {
...
function parseFilterString(expr: string, currentFile?: QuartzPluginData): FilterFn {
  ...
  // property.contains(this.file) - match frontmatter wikilink arrays against the base owner
  const propContainsThisFileMatch = trimmed.match(/^([\w.]+)\.contains\s*\(\s*this\.file\s*\)$/)
  if (propContainsThisFileMatch) {
    const propPath = propContainsThisFileMatch[1]
    return (file, _allFiles, cf) => {
      const current = cf ?? currentFile
      const propValue = getPropertyValue(file, propPath)

      if (Array.isArray(propValue)) {
        return propValue.some((item) => matchesFileRef(item, current))
      }

      return matchesFileRef(propValue, current)
    }
  }
